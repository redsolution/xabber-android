        ...
        outerloop@ while (true) {
            when (parser.eventType) {
                XmlPullParser.START_TAG -> {
                    if (parser.getAttributeValue(CccSyncManager.NAMESPACE, IncomingSetSyncIQ.STAMP_ATTRIBUTE) != null) {
                                timestamp = parser.getAttributeValue(CccSyncManager.NAMESPACE, IncomingSetSyncIQ.STAMP_ATTRIBUTE).toLong()
                    }
                    when (parser.name){
