...
class ChatListAdapter(
    ...
    fun setItems(newItemsList: MutableList<AbstractChat>) {
        list.clear()
    ...
    fun onSwipeChatItem(holder: ChatViewHolder) {
        getAbstractContactFromView(holder.itemView).let {
            ...
            listener.onChatItemSwiped(it)
        }
        if (itemCount == 0) {
            listener.onListBecomeEmpty()
        }
    }
    ...
    override fun onClick(v: View) {
        if (v.id == R.id.ivAvatar) {
            ...
            listener.onChatItemClick(list[recyclerView.getChildLayoutPosition(v)])
        }
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ChatViewHolder =
        ChatViewHolder(
            LayoutInflater.from(parent.context).inflate(
                R.layout.item_chat_in_contact_list, parent, false
            ),
            this
        )
